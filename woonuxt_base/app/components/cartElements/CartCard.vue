  ...
  <SwipeCard @remove="removeItem">
    <div v-if="productType" class="flex items-center gap-3 group">
      <NuxtLink :to="productSlug">
        <NuxtImg width="64" height="64" class="w-16 h-16 rounded-md skeleton" :src="imgScr" :alt="productType.image?.altText || productType.name" :title="productType.image?.title || productType.name" loading="lazy" />
      </NuxtLink>
      <div class="flex-1">
        ...
        <ProductPrice class="text-xs pt-2" :sale-price="productType.salePrice" :regular-price="customPrice ? `${customPrice} Ft` : productType.regularPrice" />
      </div>
      <div class="inline-flex gap-2 flex-col items-end">
        ...
        <div class="text-xs text-stone-400 flex leading-none items-center">
          ...
          <button title="Remove Item" aria-label="Remove Item" type="button" class="flex items-center gap-1 hover:text-red-500 cursor-pointer" @click="removeItem">
